...
from recipes.models import Tag, Recipe, Ingredient, FavoriteRecipe, Cart, \
    IngredientRecipe
from .serializers import (TagSerializer, IngredientSerializer,
                          RecipeSerializer,CreateUpdateRecipeSerializer,
                          ShortRecipeSerializer, UserSerializer, SubscribeSerializer,
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def set_password(self, request):
        ...
        serilizer = SetPasswordSerializer(data=request.data, context={'request': request})
    ...
    def subscribe(self, request, pk):
        ...
        serializer = SubscribeSerializer(subscribe_to, context={'request': request})
        if request.method == 'POST':
            ...
            return Response(
                ...
                status=status.HTTP_400_BAD_REQUEST)
        try:
            Subscribtion.objects.get(subscriber=request.user, subscribed_to_id=pk).delete()
    ...
    def subscriptions(self, request):
        queryset = User.objects.filter(subscribing__subscriber=self.request.user)
    ...
    def shopping_cart(self, request, pk):
        ...
        if request.method == 'POST':
            ...
            return Response({'error': 'Рецепт уже в списке покупок'},
                            status=status.HTTP_400_BAD_REQUEST)
        try:
            Cart.objects.get(recipe=recipe, user=request.user).delete()
